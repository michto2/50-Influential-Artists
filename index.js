...
function getArtistByIndex(artists, index) {
    const numArtist = artists[index];
    return `The artist at index ${numArtist.id} is ${numArtist.name}`
  }
...
/* Task 5: Create a function called get20s() that takes data as an argument and returns an array 
with names of artists who were born the 20th century ( 1900-1999) */

function get20s(artists,minYears,maxYears){

const result = artists.filter(artist => artist.years > minYears && artist.years < maxYears)
return result;
  }

  console.log(get20s(artists,1900,1999))

/* Task 6: Create a function called lotsOfArt() that takes artists as an argument and returns an array 
with names of artists who painted more than 100 paintings */

function lotsOfArt(artists){

const result = artists.filter(artist => artist.paintings > 100)
   return result;
  }

  console.log(lotsOfArt(artists))

/* Task 7: Create a function called `addArtist` that can accept an array of information and 
add it to the artists array. Then, Add a 21st artist to the array (you) with custom information! 👩‍🎨👨‍🎨
...
const newArtist = {id: 21, name:'Michelle', years: '1988 - 2020', genre: 'web', nationality: 'British', bio:'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Pretium lectus quam id leo in vitae turpis.'}
...
/* Task 8: Create a function called `checkArtist` that accepts a string (name of an artist) and 
checks if that artist is in the dataset. */

function checkArtist(artists,name){
    const data = artists.find(artist => artist.name === name)
    return data;
  }

console.log(checkArtist(artists, 'Vincent Van Gogh'))
